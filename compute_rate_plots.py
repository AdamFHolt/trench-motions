...
formulation=1 # 1 = regular, 2 = no-bending, 3 = plastic bending, 4 = power-law, 5 = plastic + power-law, 6 = simplified, 7 = power law (non-composite)

vel_converter = 0.01/(365. * 24. * 60. * 60.) ; # cm/yr to m/s 
g = 9.81;

# rheology
visc_lith = 5.0e22; visc_asthen = 5.0e20; h = 200e3;
yield_stress = 4000e6; # [MPa], used for formulation = 3
n = 3.5; trans_strain_rate = 1e-13  # [s^-1], used for formulations = 4, 5, 7 (n)
v_trans = 10.0;  # transition velocity for formulation = 7
pre = 0.8;      # slab pull prefactor for formulations = 6, 7
...
if formulation == 4 or formulation == 5:
    for i in range(0,len(plate_length_effect)):
        plate_length_effect[i,1] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,plate_length_effect[i,0],Rmin,slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    plate_length_effect[:,1] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,plate_length_effect[:,0],Rmin,slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
vc = 5.0 * vel_converter;
if formulation == 4 or formulation == 5:
    for i in range(0,len(plate_length_effect)):
        plate_length_effect[i,2] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,plate_length_effect[i,0],Rmin,slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    plate_length_effect[:,2] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,plate_length_effect[:,0],Rmin,slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
vc = 10.0 * vel_converter;
if formulation == 4 or formulation == 5:
    for i in range(0,len(plate_length_effect)):
        plate_length_effect[i,3] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,plate_length_effect[i,0],Rmin,slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    plate_length_effect[:,3] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,plate_length_effect[:,0],Rmin,slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
...
if formulation == 4 or formulation == 5:
    for i in range(0,len(slab_length_effect)):
        slab_length_effect[i,1] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,Rmin,slab_length_effect[i,0],dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    slab_length_effect[:,1] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,Rmin,slab_length_effect[:,0],dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
vc = 5.0 * vel_converter; 
if formulation == 4 or formulation == 5:
    for i in range(0,len(slab_length_effect)):
        slab_length_effect[i,2] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,Rmin,slab_length_effect[i,0],dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    slab_length_effect[:,2] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,Rmin,slab_length_effect[:,0],dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
vc = 10.0 * vel_converter; 
if formulation == 4 or formulation == 5:
    for i in range(0,len(slab_length_effect)):
        slab_length_effect[i,3] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,Rmin,slab_length_effect[i,0],dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    slab_length_effect[:,3] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,Rmin,slab_length_effect[:,0],dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
...
if formulation == 4 or formulation == 5:
    for i in range(0,len(slab_age_effect)):
        slab_age_effect[i,1] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H[i],Lsp,Rmin,slabL,dip,oceanic_buoy[i],yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    slab_age_effect[:,1] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,Rmin,slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
vc = 5.0 * vel_converter; 
if formulation == 4 or formulation == 5:
    for i in range(0,len(slab_age_effect)):
        slab_age_effect[i,2] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H[i],Lsp,Rmin,slabL,dip,oceanic_buoy[i],yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    slab_age_effect[:,2] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,Rmin,slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
vc = 10.0 * vel_converter; 
if formulation == 4 or formulation == 5:
    for i in range(0,len(slab_age_effect)):
        slab_age_effect[i,3] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H[i],Lsp,Rmin,slabL,dip,oceanic_buoy[i],yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    slab_age_effect[:,3] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,Rmin,slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
...
if formulation == 4 or formulation == 5:
    for i in range(0,len(radius_effect)):
        radius_effect[i,1] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,radius_effect[i,0],slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    radius_effect[:,1] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,radius_effect[:,0],slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
vc = 5.0 * vel_converter; 
if formulation == 4 or formulation == 5:
    for i in range(0,len(radius_effect)):
        radius_effect[i,2] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,radius_effect[i,0],slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    radius_effect[:,2] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,radius_effect[:,0],slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
vc = 10.0 * vel_converter; 
if formulation == 4 or formulation == 5:
    for i in range(0,len(radius_effect)):
        radius_effect[i,3] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,radius_effect[i,0],slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
else:
    radius_effect[:,3] = compute_vsp(formulation,vc,h,visc_asthen,visc_lith,H,Lsp,radius_effect[:,0],slabL,dip,oceanic_buoy,yield_stress,trans_strain_rate,n,pre,v_trans)
# #
...
axs[0,0].plot(plate_length_effect[:,0]/1e3,(1.0-plate_length_effect[:,1]), linewidth=1.5, color='orange')
axs[0,0].plot(plate_length_effect[:,0]/1e3,(5.0-plate_length_effect[:,2]), linewidth=1.5, color='red')
axs[0,0].plot(plate_length_effect[:,0]/1e3,(10.0-plate_length_effect[:,3]), linewidth=1.5, color='brown')
...
if depth_cutoff == 0:
    ...
    cset1=axs[0,0].scatter(data_plate_length[:,0],data_plate_length[:,1]/10.,vmin=0,vmax=10,s=8,c=colors,edgecolors='none')
    color_col=data_plate_length_deep[:,2]/10.
    colors=np.where(color_col<color_bound1,'orange',np.where(color_col<color_bound2,'red','brown'))
    cset2=axs[0,0].scatter(data_plate_length_deep[:,0],data_plate_length_deep[:,1]/10.,vmin=0,vmax=10,s=8,c=colors,edgecolors='none')
else:
...
axs[0,1].plot(slab_length_effect[:,0]/1e3,(1.0-slab_length_effect[:,1]), linewidth=1.5, color='orange')
axs[0,1].plot(slab_length_effect[:,0]/1e3,(5.0-slab_length_effect[:,2]), linewidth=1.5, color='red')
axs[0,1].plot(slab_length_effect[:,0]/1e3,(10.0-slab_length_effect[:,3]), linewidth=1.5, color='brown')
if depth_cutoff == 0:
    color_col=data_slab_length[:,2]/10.
    colors=np.where(color_col<color_bound1,'orange',np.where(color_col<color_bound2,'red','brown'))
    cset1=axs[0,1].scatter(data_slab_length[:,0],data_slab_length[:,1]/10.,vmin=0,vmax=10,s=8,c=colors,edgecolors='none')
    color_col=data_slab_length_deep[:,2]/10.
    colors=np.where(color_col<color_bound1,'orange',np.where(color_col<color_bound2,'red','brown'))
    cset2=axs[0,1].scatter(data_slab_length_deep[:,0],data_slab_length_deep[:,1]/10.,vmin=0,vmax=10,s=8,c=colors,edgecolors='none')
else:
...
axs[1,0].plot(slab_age_effect[:,0],(1.0-slab_age_effect[:,1]), linewidth=1.5, color='orange')
axs[1,0].plot(slab_age_effect[:,0],(5.0-slab_age_effect[:,2]), linewidth=1.5, color='red')
axs[1,0].plot(slab_age_effect[:,0],(10.0-slab_age_effect[:,3]), linewidth=1.5, color='brown')
if depth_cutoff == 0:
    color_col=data_slab_age[:,2]/10.
    colors=np.where(color_col<color_bound1,'orange',np.where(color_col<color_bound2,'red','brown'))
    cset1=axs[1,0].scatter(data_slab_age[:,0],data_slab_age[:,1]/10.,vmin=0,vmax=10,s=8,c=colors,edgecolors='none')
    color_col=data_slab_age_deep[:,2]/10.
    colors=np.where(color_col<color_bound1,'orange',np.where(color_col<color_bound2,'red','brown'))
    cset2=axs[1,0].scatter(data_slab_age_deep[:,0],data_slab_age_deep[:,1]/10.,vmin=0,vmax=10,s=8,c=colors,edgecolors='none')
else:
...
axs[1,1].plot(radius_effect[:,0]/1e3,(1.0-radius_effect[:,1]), linewidth=1.5, color='orange')
axs[1,1].plot(radius_effect[:,0]/1e3,(5.0-radius_effect[:,2]), linewidth=1.5, color='red')
axs[1,1].plot(radius_effect[:,0]/1e3,(10.0-radius_effect[:,3]), linewidth=1.5, color='brown')
if depth_cutoff == 0:
    color_col=data_Rmin[:,2]/10.
    colors=np.where(color_col<color_bound1,'orange',np.where(color_col<color_bound2,'red','brown'))
    cset1=axs[1,1].scatter(data_Rmin[:,0],data_Rmin[:,1]/10.,vmin=0,vmax=10,s=8,c=colors,edgecolors='none')
    color_col=data_Rmin_deep[:,2]/10.
    colors=np.where(color_col<color_bound1,'orange',np.where(color_col<color_bound2,'red','brown'))
    cset2=axs[1,1].scatter(data_Rmin_deep[:,0],data_Rmin_deep[:,1]/10.,vmin=0,vmax=10,s=8,c=colors,edgecolors='none')
else:
...
if formulation == 1:
    plot_name=''.join(['plots/rates_',str(vt_ref),'model_',depths_string,'_LithVisc',str(visc_lith),'_AsthVisc',str(visc_asthen),'.png'])
elif formulation == 2:
    plot_name=''.join(['plots/rates_',str(vt_ref),'model_',depths_string,'_LithVisc',str(visc_lith),'_AsthVisc',str(visc_asthen),'.no_bending.png'])
elif formulation == 3:
    plot_name=''.join(['plots/rates_',str(vt_ref),'model_',depths_string,'_LithVisc',str(visc_lith),'_AsthVisc',str(visc_asthen),'.plastic_bending.png'])
elif formulation == 4:
    plot_name=''.join(['plots/rates_',str(vt_ref),'model_',depths_string,'_LithVisc',str(visc_lith),'_AsthVisc',str(visc_asthen),'_Trans',str(trans_strain_rate),'.power-law.png'])
elif formulation == 5:
    plot_name=''.join(['plots/rates_',str(vt_ref),'model_',depths_string,'_YieldStress',str(yield_stress/1e9),'GPa_AsthVisc',str(visc_asthen),'_Trans',str(trans_strain_rate),'.plastic-bending_and_power-law.png'])
elif formulation == 6:
    plot_name=''.join(['plots/rates_',str(vt_ref),'model_',depths_string,'_SPpre',str(pre),'_AsthVisc',str(visc_asthen),'.SlabPullPrefactor.png'])
elif formulation == 7:
