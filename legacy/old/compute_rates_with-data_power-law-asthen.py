...
Rmin = 250e3; 
visc_lith = 500.0e20; visc_asthen_newt = 1.0e21; h = 200e3;
...
n = 3.5; trans_strain_rate = 5e-15; 

### effect of plate length (Lsp)
plate_length_effect = np.zeros((101,4))
plate_length_effect[:,0] = np.linspace(2000e3,12000e3,101)
vc = 1.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
plate_length_effect[:,1] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/plate_length_effect[:,0])**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 5.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
plate_length_effect[:,2] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/plate_length_effect[:,0])**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h))  ) ** n) * (1.0/vel_converter)
vc = 10.0 * vel_converter;
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
plate_length_effect[:,3] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/plate_length_effect[:,0])**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h))  ) ** n) * (1.0/vel_converter)

print plate_length_effect[:,3]
...
vc = 1.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
slab_dip_effect[:,1] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(slab_dip_effect[:,0]))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(slab_dip_effect[:,0]))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 5.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
slab_dip_effect[:,2] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(slab_dip_effect[:,0]))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(slab_dip_effect[:,0]))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 10.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
slab_dip_effect[:,3] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(slab_dip_effect[:,0]))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(slab_dip_effect[:,0]))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
...
vc = 1.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
slab_age_effect[:,1] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 5.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
slab_age_effect[:,2] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 10.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
slab_age_effect[:,3] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
...
visc_asthen_slab =  asthen_effect[:,0] * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
vc = 1.0 * vel_converter; 
asthen_effect[:,1] = ((((2*h*trans_strain_rate)**(1-n))/(asthen_effect[:,0]**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 5.0 * vel_converter; 
visc_asthen_slab =  asthen_effect[:,0] * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
asthen_effect[:,2] = ((((2*h*trans_strain_rate)**(1-n))/(asthen_effect[:,0]**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 10.0 * vel_converter; 
visc_asthen_slab =  asthen_effect[:,0] * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
asthen_effect[:,3] = ((((2*h*trans_strain_rate)**(1-n))/(asthen_effect[:,0]**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
...
vc = 1.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
depth_effect[:,1] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((depth_effect[:,0] * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (depth_effect[:,0]/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 5.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
depth_effect[:,2] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((depth_effect[:,0] * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (depth_effect[:,0]/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 10.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
depth_effect[:,3] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((depth_effect[:,0] * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/Rmin**3) * visc_lith * vc) - \
	 (2.0 * vc * (depth_effect[:,0]/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)

### effect of radius of curvature
radius_effect = np.zeros((101,4))
radius_effect[:,0] = np.linspace(10e3,700e3,101)
vc = 1.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
radius_effect[:,1] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/radius_effect[:,0]**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 5.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
radius_effect[:,2] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/radius_effect[:,0]**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
vc = 10.0 * vel_converter; 
visc_asthen_slab = visc_asthen_newt * ((vc)/(2 * h * trans_strain_rate))**((1-n)/n)
radius_effect[:,3] = ((((2*h*trans_strain_rate)**(1-n))/(visc_asthen_newt**n)) * ((h/Lsp)**n) * ( ((D * oceanic_buoy * g)/np.sin(np.deg2rad(dip))) - ((2./3.) * (H**3/radius_effect[:,0]**3) * visc_lith * vc) - \
	 (2.0 * vc * (D/np.sin(np.deg2rad(dip))) * (visc_asthen_slab/h)) ) ** n) * (1.0/vel_converter)
...
plt.savefig('test_with-data_power-law-asthen.png', bbox_inches='tight')
