...
USAGE = """Usage:
  python3 compute_rates_single.py <vt_ref> <formulation> <DP_ref> <asthen_visc> <lith_visc> [--skip-map] [--out-prefix <dir>]
...
if cfg:
	...
	asthen_visc = float(main_args[3])
	lith_visc = float(main_args[4])
	skip_map = False
	out_prefix = ''
...
segments = build_segment_arrays(data, data_vt, vt_col, vel_converter, max_age)
n = segments['n']
num = segments['num']
Lsp = segments['Lsp']
dip = segments['dip']
